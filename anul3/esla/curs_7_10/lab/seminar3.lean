...
| atom => false
| neg k => containsBox' k
| impl k r => (containsBox' k) || (containsBox' r)
| box k => true
...
notation Γ "∪" p => p :: Γ
notation p "∈s" Γ => List.element Γ p 
...
-- 1 :: 2 :: 3 :: []
example : 1 ∈s L := by constructor 
example : 2 ∈s L := by repeat constructor
example : 3 ∈s L := by repeat constructor 
...
example { p q : form } : ⊢ □p → □(q → p) :=
  have h₂: ⊢ (p → (q → p)) := pl1 
  have h₃: ⊢ □(p → (q → p)) := nec h₂
  have h₄: ⊢ □(p → (q → p)) → (□p → □(q → p)) := k
  show ⊢ □p → □(q → p) from mp h₄ h₃
